...
; implementation of the DEB-infection equations of Civitello et al. XXXX in an IBM
...
globals[
  ...
  resource_type ; type of food source: detritus or algae
...
snails-own[
  ...
  ee         ; scaled reserve density of hosts
  D           ; host development
  RH         ; energy in reproduction buffer
  ...
  L-min       ; minimum shell length (host)
  L-max       ; maximum shell length (host)
]
...
to setup
  ...
  ask  snails  [
    set L-min 4 ; 4 mm length
    set L-max 16 ; 16 mm length
    set L L-min + random-float(L-max - L-min) ; random draw between min and max length ; random-float 2 + 4
    set ee 0.9
    set D 0
    set RH 0
    set P 0
...
;  create-adult count snails with [RH > 0]
  ...
  ask patches [
    ...
    set M 0
    set Z 0
    set G 0]; set initial value of prey to their carrying capacity
  reset-ticks
end
...
to am-I-dead
  if ee <= 0 [die]
  if random-float 1 <= (1 - exp (HAZ * -1)) [die] ; kill hosts when hazard rate becomes too high
end
...
### 15-1-19  
> added L-min and L-max shell length to randomise initial length of hosts 
> added detritus and algae options as resource type 

...
# 4-3-20
# add 'nocontrol' to file handle for nocontrol==1 sims 
...
n.ticks <- 1000 # set number of days to simulate
...
no_control <- 1 # run normal no control sims

# snail control
me_events <- n.ticks+1 # 1 = on day 30, every month, skip a month. 8 = hailmary
...
# for @productivity == 1 # need to comment out rg and detr in next loop    
# for(rg in rg_pars){ # loop through rgs (food growth rates)
# for(detr in detr_pars){ # loop through det (food growth rates)

# for(snail_snack_min in snail_snack_min_vec){ # @snailsnack
# for(snail_snack_max in snail_snack_max_vec){ # @snailsnack
      ...
      for(me_im_event in me_im_events){ # 1:5 # me impact (0.69, 1.39, 2.3 ...)
        for(me_event in me_events){ # 1:7 # run all me event scenarios and save to file.
          ...
          if(snail_control == 1){
            ...
            if(me_event==n.ticks+1){me_days <- n.ticks + 1; me_fh = ""}
          ...
          if(biocontrol==1){
            if(productivity==1){ # for altering rg and det values
              ...
              global_output_fh = paste0(wd,"/biocontrol_sims/",init_host_pop,"/new_new_pred_a/productivity/productivity_0_25/pred_05_100/",fhh,".R") 
            }else{
          ...
          if(no_control==1){
            # no control scenario
            fhh = paste0(resource_type,"_",me_fh,"_meim",me_im_event,"_rep",rn);fhh
            global_output_fh = paste0(wd,"/nocontrol/150days_plus/",fhh,".R")
          }

...
# version 1.4 (molluscicide events and impact)
...
me_event <- 2 # 1 = every 2 weeks, 2 = every month, 3 = every 2 months 
me_im_event <- 5 # 1 = 0.69, 2 = 1.39, 3 = 2.30, 4 = 3.0, 5 = 4.60 for me_im
resource_type="detritus" # detritus # set resource type
rep_num <- 3 # number of replications

#################################

# for algae and det 
for(rn in 1:3){ # reps
  for(me_im_event in 1){ # run all five me_im_event scenarios and save to file. 
    ...
    me_im = 4.6
    global_output_fh = paste0(wd,"/global_output_",resource_type,"_",resources,"_me",me_event,"_meim",me_im_event,"_every30days.R"); global_output_fh
    # global_output_fh = paste0(wd,"/global_output_",resource_type,"_",resources,"_grantprop.R"); global_output_fh
...
} # end reps for me_im_event sims 

me_im_event = 1 # choose which me impact level to plot
rn=1 # choose rep number
...
setwd("/Users/malishev/Documents/Emory/research/schisto_ibm/nih_grant")
...
par(mfrow=c(2,2))
plot(mm_[[12]][[1]],mm_[[1]][[1]],type="l") # plot cercs and infected host length
plot(mm_[[1]][[1]],type="l") 

...
me_event <- 8 # 1 = on day 30, every month, skip a month, etc
...
be_event <- 1 # 2 3 selevct biocontrol event (diff snail snack size)
init_host_pop = 60 # initial population size
init_host_pop_vec <- 60 # c(50,100,200,500,1000)
...
for(rn in 1:rep_num){
  ...
  for(me_im_event in 1:5){ # 1:5 # me impact (0.69, 1.39, 2.3 ...)
    for(me_event in 8){ # 1:7. run all me event scenarios and save to file.
      ...
      if(snail_control == 1){
        ...
        if(me_event==8){me_days = 120; me_fh = paste0("hailmary",me_days)}
        if(me_event==151){me_days <- n.ticks + 1; me_fh = me}
      ...
      # file handles   ----------------------------------------------------------
      if(snail_control==1){    
        # global_output_fh = paste0(wd,"/me_detr_sims/",fhh,".R")
      ...
      if(biocontrol==1){
        fhh = paste0(resource_type,"_be",be_event,"_init",init_host_pop);fhh
          ...
          for(alpha in alpha_pars){ # loop through alphas (amplitude in food cycle)
            for(rho in rho_pars){ # loop through rhos (periodicity of food cycle)
              for(rg in rg_pars){ # loop through rgs (food growth rates)
                for(me in me_pars){ # loop through me (molluscicide day events)
                  for(me_im in me_im_pars){ # loop through me_im (molluscicide impact events)
                    ...
                    for(t in 1:n.ticks){ # start nl sim  @netlogo
                      ...
                      if(NLReport("count snails") > 0){
                        ...
                        # @biocontrol -------------------------------------------------- 
                        if(biocontrol == 1){
                          if(any(snail.stats$L >= snail_snack) == T){
                            ...
                            # cat("\nsnail length = ", unique(snail.stats$L))
                          } 
                        }
...
# global_output_fh = "R:/CivitelloLab/matt/schisto_ibm/plos_sims/skip30/detritus_skip30_meim2_rep2.R"

mm_ = readRDS(global_output_fh)
layout(matrix(c(1:16),4,4,byrow=T))
# plot master
mm <- mm_[[3]] # 1=cercs, 2=food
y_m <- melt(mm);y_m
ggplot() +

---
title: Instructions for runnng the individual-based Dynamic Energy Budget _Schistosoma_ popultpn model (SIDEB) on Mac OSX     
author: |
...
If you get an error message beginning with 'Failed Test ...', move onto ["Install JGR"](#install-jgr).  

### Running `RNetLogo`  

Load the 'DEB_IBM.R' file into your `R` session. 

### Install `rJava`  
\  
If an `rJava` error occurs in your `R` session, open Terminal (Applications > Terminal) and run the following in Terminal (sourced from [[1]](#references) and [[2]](#references)) 
\  
```{bash,eval=F}
...
Now load the 'DEB_IBM.R' file into your `R` session to run the simulation model.   
